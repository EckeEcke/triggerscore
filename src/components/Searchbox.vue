    ...
    <div class="bg-cover" :class="showTitle ? 'py-12' : 'py-4'" :style="{'background-image': 'linear-gradient(rgba(220, 0, 0, 0.6), rgba(220, 0, 100, 0.6))'}">
        <div class="container mx-auto px-4 xl:w-10/12">
            ...
            <div class="flex mx-auto py-4 self-center w-full opacity-95">
                <div class="rounded-lg flex w-full justify-start focus-within:shadow-2xl shadow-blue-900 shadow-inner">
                ...
                <input type="text" id="search" v-model="searchInput" v-on:keyup.enter="searchMovie" @input="resetSearchResults" class="px-4 py-4 w-full rounded-r-xl outline-none transition scroll-my-4 focus:bg-red-50" style="scroll-margin-top: 10rem;" :placeholder="$t('header.searchPlaceholder')">
                </div>
            </div>
        </div>
        </div>
