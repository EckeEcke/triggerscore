<template>
    <router-link :to="{name: 'movie', params: {id: movie.id}}" tag="div" class="w-40 h-56 mx-2 bg-white shadow-md flex flex-none flex-col relative rounded-lg transform transition duration-300 md:hover:scale-105 container-xl cursor-pointer">
        <div class="flex">
            ...
            <div class="mx-auto mt-3 h-12 w-12 text-white rounded-lg" :class="{'bg-gray-200': !scoreAvailable, 'bg-red-700': scoreAvailable && scores.rating_total >= 7, 'bg-yellow-400': scoreAvailable && scores.rating_total < 7 && scores.rating_total >=4, 'bg-green-500': scoreAvailable && scores.rating_total < 4}">
