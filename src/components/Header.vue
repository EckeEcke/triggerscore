<template>
    <header class="w-full h-auto bg-gradient-to-b from-red-700 to red-600 to-red-600 z-10 shadow-md flex flex-col relative sticky top-0">
        <section class="container mx-auto h-full p-4 xl:w-10/12 flex justify-between relative">
            ...
            <div class="flex">
              <router-link to="/about" tag="a" class="text-white font-semibold self-center mr-10 hidden sm:block">About</router-link>
              <router-link to="/faq" tag="a" class="text-white font-semibold self-center mr-10 hidden sm:block">FAQ</router-link>
              <font-awesome-icon :icon="['fas', 'search']" class="text-lg text-yellow-300 mr-10 self-center" @click="showSearch = !showSearch" />
              <font-awesome-icon :icon="['fas', 'ellipsis-v']" class="text-lg text-yellow-300 self-center" @click="showMenu = !showMenu" />
            </div>
